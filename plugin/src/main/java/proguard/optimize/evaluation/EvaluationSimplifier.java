...
//            method.getName(clazz).equals("abc");
    ...
    private void deleteReferencePopInstruction(Clazz clazz,
        ...
        if (partialEvaluator.isSubroutineStart(offset) &&
            ...
            if (DEBUG)
                System.out.println("  Deleting store of subroutine return address " + instruction.toString(offset));
    ...
    private void replaceBranchInstruction(Clazz clazz,
        ...
        if (branchTargets != null &&
            ...
            if (branchOffset == instruction.length(offset)) {
                if (DEBUG)
                    System.out.println("  Ignoring zero branch instruction at [" + offset + "]");
            } else {
    ...
    private void replaceByInfiniteLoop(Clazz clazz,
        ...
        if (DEBUG)
            System.out.println("  Replacing unreachable instruction by infinite loop " + replacementInstruction.toString(offset));
    ...
    private void replaceInstruction(Clazz clazz,
        ...
        if (DEBUG)
            System.out.println("  Replacing instruction " + instruction.toString(offset) + " -> " + replacementInstruction.toString() + (popCount == 0 ? "" : " (" + popCount + " pops)"));
    ...
    private void replaceSimpleEnumSwitchInstruction(Clazz clazz,
        ...
        if (DEBUG)
            System.out.println("  Replacing switch instruction at [" + switchOffset + "] -> [" + loadOffset + "] swap + pop, " + replacementSwitchInstruction.toString(switchOffset) + ")");
