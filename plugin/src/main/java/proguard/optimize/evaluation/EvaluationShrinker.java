    ...
    private class MyStackConsistencyFixer
            ...
            if (isInstructionNecessary(offset)) {
                ...
                if (popCount > 0) {
                    ...
                    if (requiredPopCount > 0) {
                        if (DEBUG)
                            System.out.println("  Inserting before marked consumer " + instruction.toString(offset));
                ...
                if (pushCount > 0) {
                    ...
                    if (requiredPopCount > 0) {
                        if (DEBUG)
                            System.out.println("  Inserting after marked producer " + instruction.toString(offset));
                ...
                if (popCount > 0) {
                    ...
                    if (expectedPopCount > 0) {
                        if (DEBUG)
                            System.out.println("  Replacing unmarked consumer " + instruction.toString(offset));
                ...
                if (pushCount > 0) {
                    ...
                    if (expectedPushCount > 0) {
                        if (DEBUG)
                            System.out.println("  Replacing unmarked producer " + instruction.toString(offset));
            ...
            if (isInstructionNecessary(offset) &&
                ...
                if ((newOpcodes & ~0xff) == 0) {
                    ...
                    if (newOpcode == InstructionConstants.OP_NOP) {
                        ...
                        if (DEBUG)
                            System.out.println("  Deleting marked instruction " + simpleInstruction.toString(offset));
                    } else if (newOpcode == oldOpcode) {
                        // Leave the instruction unchanged.
                        codeAttributeEditor.undeleteInstruction(offset);

                        if (DEBUG)
                            System.out.println("  Marking unchanged instruction " + simpleInstruction.toString(offset));
                    } else {
                        ...
                        if (DEBUG)
                            System.out.println("  Replacing instruction " + simpleInstruction.toString(offset) + " by " + replacementInstruction.toString());
                    }
                } else {
                    ...
                    if (DEBUG)
                        System.out.println("  Replacing instruction " + simpleInstruction.toString(offset) + " by");
                    int count = 0;
                    while (newOpcodes != 0) {
        ...
        private int fixDupSwap(int instructionOffset,
            ...
            switch (dupSwapOpcode) {
                case InstructionConstants.OP_DUP:
                    return fixedDup(instructionOffset, topBefore, topAfter);
                case InstructionConstants.OP_DUP_X1:
                    return fixedDup_x1(instructionOffset, topBefore, topAfter);
                case InstructionConstants.OP_DUP_X2:
                    return fixedDup_x2(instructionOffset, topBefore, topAfter);
                case InstructionConstants.OP_DUP2:
                    return fixedDup2(instructionOffset, topBefore, topAfter);
                case InstructionConstants.OP_DUP2_X1:
                    return fixedDup2_x1(instructionOffset, topBefore, topAfter);
                case InstructionConstants.OP_DUP2_X2:
                    return fixedDup2_x2(instructionOffset, topBefore, topAfter);
                case InstructionConstants.OP_SWAP:
                    return fixedSwap(instructionOffset, topBefore, topAfter);
                default:
    ...
    private void markVariableInitializersAfter(int producerOffset,
        ...
        if (!isInstructionNecessary(producerOffset)) {
            ...
            if (isVariableInitialization(producerOffset, variableIndex)) {
                // Mark the producer.
                if (DEBUG)
                    System.out.print("  Marking initialization of v" + variableIndex + " at ");
