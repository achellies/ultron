...
public class LookUpSwitchInstruction extends SwitchInstruction {
    ...
    protected void readInfo(byte[] code, int offset) {
        ...
        defaultOffset = readInt(code, offset);
        offset += 4;
        int jumpOffsetCount = readInt(code, offset);
        offset += 4;
        ...
        for (int index = 0; index < jumpOffsetCount; index++) {
            cases[index] = readInt(code, offset);
            offset += 4;
            jumpOffsets[index] = readInt(code, offset);
            offset += 4;
        }
    }
    ...
    protected void writeInfo(byte[] code, int offset) {
        ...
        writeInt(code, offset, defaultOffset);
        offset += 4;
        writeInt(code, offset, cases.length);
        offset += 4;

        // Write the matches-offset pairs.
        for (int index = 0; index < cases.length; index++) {
            writeInt(code, offset, cases[index]);
            offset += 4;
            writeInt(code, offset, jumpOffsets[index]);
            offset += 4;
        }
    }
